...
fn test_offline_queueing_persistence() {
    ...
    assert!(
        queue_file.exists(),
        "pending_queue.json must be persisted to disk"
    );

    let content = std::fs::read_to_string(&queue_file).unwrap();
    assert!(
        content.contains("Are you there?"),
        "Persisted queue must contain the user message"
    );
    ...
    // 두 번째 세션: 새로운 세션 시작 시 파일에서 큐를 정상적으로 복구하는지 확인
    {
        ...
        assert_eq!(
            session.pending_count(),
            1,
            "Queue should be reloaded from disk"
        );
    ...
    assert!(
        content2.contains("Second failing message"),
        "Second message must be saved"
    );
}
